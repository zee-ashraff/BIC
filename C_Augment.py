...
image_list_train = pd.read_csv(str(parent_directory) + "/" + "temp_augment.csv")
...
for i in image_list_train.index:
    ...
    flip_hr_path = parent_directory +"/training_set_4/negative/"+ str(image_list_train['filename'][i])+'H.jpg'
    imageio.imwrite(flip_hr_path, flip_hr_image)

     #vertical flip
    flip_vr = iaa.Flipud(p=1.0)
    flip_vr_image = flip_vr.augment_image(image)
    flip_vr_path = parent_directory +"/training_set_4/negative/"+ str(image_list_train['filename'][i])+'V.jpg'
    imageio.imwrite(flip_vr_path, flip_vr_image)

    #noise added
    gaussian_noise = iaa.AdditiveGaussianNoise(15, 20)
    noise_image = gaussian_noise.augment_image(image)
    noise_path = parent_directory +"/training_set_4/negative/"+ str(image_list_train['filename'][i])+'N.jpg'
